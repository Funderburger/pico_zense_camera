...
private:
    ...
    ros::NodeHandle private_nh, colour_nh_, depth_nh_, aligned_nh_, imu_nh_, ir_nh_;
    std::shared_ptr<image_transport::ImageTransport> colour_it_, depth_it_, aligned_it_, ir_it_;
    std::shared_ptr<camera_info_manager::CameraInfoManager> colour_info_, depth_info_, aligned_info_;

    image_transport::CameraPublisher colour_pub_, depth_pub_, aligned_pub_, ir_pub_;
    ros::Publisher imu_pub_;

    int32_t device_count_, device_index_, data_mode_ = PsDepthAndIRAndRGB_30;//PsDepthAndRGB_30;
